...
for reg in regs:
    if len(reg.fields) > 1:
        ...
        for field in reg.get_sorted_fields():
            ...
            param['fields'].append((field.label, field.size))
            last_start = field.msb + 1

        print(typedef_tpl.render(**param))
...
void LTC4015_print_{{name}}(LTC4015_{{name}}_TypeDef reg) {
    shell_printf("{{name}} = 0x%04x\\n", reg);
{%- for field in fields %}
    {%- if field.size == 1 %}
    shell_printf(" .{{field.label}} = %d\\n", reg.fields.{{field.label}});
    {%- else %}
    shell_printf(" .{{field.label}} = 0x%04x\\n", reg.fields.{{field.label}});
    {%- endif %}
{%- endfor %}
}
""")
